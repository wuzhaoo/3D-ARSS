...
class ChannelAttention(ME.MinkowskiNetwork):
    ...
    def forward(self, x):
        ...
        conv = nn.Conv1d(1, 1, kernel_size=self.k, padding=int(self.k / 2), bias=False)
        conv=conv.cuda()
        max_pool_out = torch.max(x.F, dim=0)[0].unsqueeze(dim=0)
        avg_pool_out = torch.mean(x.F, dim=0).unsqueeze(dim=0)
        avg_pool_out=avg_pool_out.unsqueeze(dim=0)
        avg_fc_out = conv(avg_pool_out)
        avg_fc_out = avg_fc_out.squeeze(dim=0)
        max_pool_out = ME.SparseTensor(
            features=max_pool_out,
            coordinates=torch.Tensor([[0, 0, 0, 0]]).int().cuda()
        )
        avg_fc_out = ME.SparseTensor(
            features=avg_fc_out,
