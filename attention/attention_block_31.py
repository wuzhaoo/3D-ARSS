...
class ChannelAttention(ME.MinkowskiNetwork):
    def __init__(self, in_planes,D,ratio=16,gamma=2,b=1):
        ...
        self.conv1 = ME.MinkowskiConvolution(in_planes, in_planes // 16, self.k, dimension=3)
        self.relu = ME.MinkowskiReLU()
        self.conv2 = ME.MinkowskiConvolution(in_planes//16, in_planes, self.k, dimension=3)
        # self.avg = ME.MinkowskiAvgPooling(kernel_size=2,stride=2,dimension=3)
        self.avg = ME.MinkowskiGlobalAvgPooling()
        self.max = ME.MinkowskiGlobalMaxPooling()
        self.sigmoid = ME.MinkowskiSigmoid()


    def forward(self, x):
        # max_pool_out = self.max(x)
        # avg_pool_out = self.avg(x)
        a=[]
        ...
        avg_fc_out=self.relu(avg_fc_out)
        
        avg_fc_out = self.conv2(avg_fc_out)
    
        # max_fc_out = self.conv2(max_pool_out)
        
        max_fc_out=self.conv1(max_pool_out)
        
        max_fc_out=self.relu(max_fc_out)
        max_fc_out = self.conv2(max_fc_out)
        # a.append(feat_1.shape)
        out = avg_fc_out+max_fc_out
        # out=ME.cat([avg_fc_out, max_fc_out])
        out = self.sigmoid(out)
        # out =x*out
        out_feat = out.F * x.F
        # feat = x.F.permute(1,0)
        # out_feat = torch.matmul(out.F,feat)
        out = ME.SparseTensor(
            features=out_feat,
